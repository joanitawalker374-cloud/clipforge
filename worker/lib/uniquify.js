...
const { formatFilter, watermarkFilter } = require("./media");
...
function uniquify(input, output, opts = {}) {
  ...
  const args = ["-y", "-threads", "1", "-i", input];

  if (level === "meta") {
    // Juste ré-écriture des métadonnées + remux (rapide, pas de ré-encodage visuel).
